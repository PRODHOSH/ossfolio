# Code of Conduct

## Workflow

```text
Code of Conduct
│
├── Our Commitment
│   └── What the project stands for
│
├── Scope
│   └── Where this Code of Conduct applies
│
├── Our Standards
│   ├── Positive behaviour
│   └── Unacceptable behaviour
│
├── Enforcement
│   ├── How to report
│   ├── How reports are handled
│   └── Possible actions
│
└── Attribution
```

## Our Commitment

We are committed to fostering a welcoming, inclusive, and respectful community where everyone can contribute, collaborate, and learn in a safe environment. We expect all members, contributors, maintainers, and participants to treat one another with professionalism, courtesy, and respect, regardless of age, body size, disability, ethnicity, gender identity or expression, level of experience, nationality, personal appearance, race, religion, sexual orientation, or any other personal characteristic.

## Scope

This Code of Conduct applies to all project spaces, including GitHub Issues, Pull Requests, Discussions, project documentation, and any other official communication channels related to OSSfolio.

It also applies when an individual is officially representing the project in public spaces, such as community events or online discussions.```text

## Our Standards

### Examples of positive behavior include:

- Being respectful, considerate, and welcoming to others.
- Using inclusive and professional language in all interactions.
- Respecting differing opinions, experiences, and perspectives.
- Providing and accepting constructive feedback gracefully.
- Collaborating in good faith and focusing on what benefits the community.
- Helping create a friendly, supportive, and harassment-free environment.

### Examples of unacceptable behavior include:

- Harassment, bullying, intimidation, or discrimination of any kind.
- Trolling, insulting, derogatory comments, or personal attacks.
- Hate speech or offensive remarks targeting individuals or groups.
- Sharing another person's private or confidential information without explicit permission.
- Unwelcome sexual attention, language, imagery, or advances.
- Deliberately disrupting discussions, events, or project activities.
- Any conduct that would reasonably be considered inappropriate in a professional or collaborative environment.

## Enforcement

Project maintainers are responsible for enforcing this Code of Conduct fairly and consistently. Reports of unacceptable behavior will be reviewed promptly, confidentially, and impartially.

If you experience or witness behavior that violates this Code of Conduct, please report it by contacting the project maintainers through the appropriate communication channels. All reports will be handled with discretion, and appropriate corrective actions may be taken depending on the severity of the violation.

Maintainers who fail to uphold or enforce this Code of Conduct may also be subject to appropriate action as determined by the project leadership.

## Attribution

This Code of Conduct is adapted from the **Contributor Covenant**, version 2.1.

For more information, visit: https://www.contributor-covenant.org

code of conduct design workflow
