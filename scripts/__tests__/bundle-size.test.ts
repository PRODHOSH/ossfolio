...
describe('measureBundle', () => {
  ...
  it('totals match the sum of the files', async () => {
    const result = await measureBundle(staticDir);
    const raw = result.files.reduce(
      (s: number, f: { bytes: number }) => s + f.bytes,
      0,
    );
    const gz = result.files.reduce(
      (s: number, f: { gzipBytes: number }) => s + f.gzipBytes,
      0,
    );
...
describe('diffBundles', () => {
  ...
  it('reports shrinkage as a negative delta', () => {
    const diff = diffBundles(
      report([['chunks/a-11111111.js', 200]]),
      report([['chunks/a-22222222.js', 150]]),
    );
    expect(diff.totalGzipDelta).toBe(-50);
  ...
  it('matches chunks across builds despite differing hashes', () => {
    const diff = diffBundles(
      report([['chunks/main-aaa11111.js', 100]]),
      report([['chunks/main-bbb22222.js', 120]]),
    );
  ...
  it('marks a genuinely new chunk as added', () => {
    const diff = diffBundles(
      report([]),
      report([['chunks/new-11111111.js', 90]]),
    );
  ...
  it('marks a deleted chunk as removed', () => {
    const diff = diffBundles(
      report([['chunks/old-11111111.js', 90]]),
      report([]),
    );
  ...
  it('omits unchanged chunks from the change list', () => {
    const diff = diffBundles(
      report([
        ['chunks/a-11111111.js', 100],
        ['chunks/b-11111111.js', 50],
      ]),
      report([
        ['chunks/a-22222222.js', 100],
        ['chunks/b-22222222.js', 80],
      ]),
    );
    expect(diff.changes).toHaveLength(1);
  ...
  it('orders changes by magnitude regardless of direction', () => {
    const diff = diffBundles(
      report([
        ['chunks/a-11111111.js', 100],
        ['chunks/b-11111111.js', 500],
      ]),
      report([
        ['chunks/a-22222222.js', 110],
        ['chunks/b-22222222.js', 100],
      ]),
    );
  ...
  it('sums chunks that normalise to the same logical name', () => {
    ...
    const diff = diffBundles(
      report([
        ['chunks/main-aaaaaaaa.js', 100],
        ['chunks/main-bbbbbbbb.js', 40],
      ]),
...
describe('renderMarkdownReport', () => {
  ...
  it('reports no change when totals match', () => {
    ...
    expect(renderMarkdownReport(same)).toContain(
  ...
  it('collapses a long change list and says how many were hidden', () => {
    const many = Array.from(
      { length: 30 },
      (_, i) => [`chunks/c${i}-11111111.js`, 100] as [string, number],
    );
    const after = Array.from(
      { length: 30 },
      (_, i) => [`chunks/c${i}-22222222.js`, 100 + i + 1] as [string, number],
    );
  ...
  it('honours a noise threshold', () => {
    const md = renderMarkdownReport(
      diffBundles(
        report([['chunks/a-11111111.js', 1000]]),
        report([['chunks/a-22222222.js', 1005]]),
      ),
      { thresholdBytes: 100 },
    );
  ...
  it('labels added and removed chunks in the table', () => {
    const diff = diffBundles(
      report([
        ['chunks/keep-11111111.js', 500],
        ['chunks/gone-11111111.js', 80],
      ]),
      report([
        ['chunks/keep-22222222.js', 500],
        ['chunks/fresh-33333333.js', 60],
      ]),
    );
