...
describe('stripComments', () => {
  it('removes line comments', () => {
    expect(
      stripComments('-- create table ghost (x int);\nselect 1;'),
    ).not.toMatch(/ghost/);
  });

  it('removes block comments', () => {
    expect(
      stripComments('/* create table ghost (x int); */ select 1;'),
    ).not.toMatch(/ghost/);
  });
});
...
describe('extractCreatedTables', () => {
  ...
  it('handles IF NOT EXISTS and a public. prefix', () => {
    expect(
      extractCreatedTables(
        'CREATE TABLE IF NOT EXISTS public.snapshots (id int);',
      ),
    ).toEqual(['snapshots']);
  });
  ...
  it('handles quoted identifiers', () => {
    expect(
      extractCreatedTables('create table "public"."my_table" (id int);'),
    ).toEqual(['my_table']);
  });
