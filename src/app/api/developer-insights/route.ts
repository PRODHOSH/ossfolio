...
import {
  ...
  developerInsightsPrompt,
  parseDeveloperInsights,
  parseDeveloperInsightsProfile,
} from '@/lib/developer-insights';
...
async function generateWithAnthropic(
  ...
  return fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1400,
      temperature: 0.3,
      system: DEVELOPER_INSIGHTS_SYSTEM_PROMPT,
...
async function generateWithOpenAI(
  ...
  return fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: DEVELOPER_INSIGHTS_SYSTEM_PROMPT },
...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit.allowed)
    return createErrorResponse(
      'Too many insight requests. Please try again shortly.',
      429,
  ...
  if (!apiKey || (provider !== 'anthropic' && provider !== 'openai'))
    return createErrorResponse('Developer Insights is not configured.', 503);
  ...
  const profile = parseDeveloperInsightsProfile(
  ...
  try {
    ...
    if (!response.ok)
      return createErrorResponse(
        'Developer Insights is temporarily unavailable.',
        502,
      );
    ...
    try {
      insights = parseDeveloperInsights(JSON.parse(content));
    } catch {
      insights = null;
    }
    if (!insights)
