...
import { fetchProfileEndorsements, toggleEndorsement } from "@/lib/endorsements";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const endorsements = await fetchProfileEndorsements(username, currentUserId);
...
export async function POST(request: NextRequest) {
  try {
    const token = extractToken(request);
    if (!token) {
      return createErrorResponse("Unauthorized - please sign in to endorse skills", 401);
    }
    ...
    if (!username || !skill) {
      return createErrorResponse("Missing required fields (username, skill)", 400);
    }
    ...
    if (!result.success) {
      return createErrorResponse(result.error || "Failed to update endorsement", 400);
    }

    const updatedEndorsements = await fetchProfileEndorsements(username, user.id);
