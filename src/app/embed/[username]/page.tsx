...
interface EmbedPageProps {
  ...
  searchParams: Promise<{ theme?: string; compact?: string; showHeatmap?: string }>;
}
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          />
          <div>
            <div style={{ fontSize: isCompact ? "15px" : "18px", fontWeight: 700, margin: 0 }}>
              @{username}
            </div>
            <div style={{ fontSize: "12px", color: muteColor, marginTop: "2px" }}>
      ...
      >
        <div style={{ backgroundColor: cardBg, padding: "8px 10px", borderRadius: "8px", textAlign: "center" }}>
          <div style={{ fontSize: "16px", fontWeight: 700 }}>{stats.totalCommits}</div>
          <div style={{ fontSize: "10px", color: muteColor }}>Commits</div>
        </div>
        <div style={{ backgroundColor: cardBg, padding: "8px 10px", borderRadius: "8px", textAlign: "center" }}>
          <div style={{ fontSize: "16px", fontWeight: 700 }}>{stats.totalPRs}</div>
          <div style={{ fontSize: "10px", color: muteColor }}>PRs</div>
        </div>
        <div style={{ backgroundColor: cardBg, padding: "8px 10px", borderRadius: "8px", textAlign: "center" }}>
          <div style={{ fontSize: "16px", fontWeight: 700 }}>{stats.totalIssues}</div>
      ...
      {!isCompact && displayHeatmap && (
        <div style={{ marginTop: "12px" }}>
          <div style={{ fontSize: "11px", color: muteColor, marginBottom: "6px" }}>Recent Activity</div>
            ...
            {heatmapBlocks.map((lvl, idx) => (
              <div
                ...
                style={{
                  ...
                  backgroundColor:
                    lvl === 0
                      ? isLight ? "#ebedf0" : "rgba(255, 255, 255, 0.08)"
                      : lvl === 1
                      ? "#a5b4fc"
                      : lvl === 2
                      ? "#818cf8"
