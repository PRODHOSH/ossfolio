...
export function ImpactNetwork({
  ...
  const [filterType, setFilterType] = useState<"all" | "repo" | "org" | "collaborator">("all");
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterType !== "all") {
      nodes = nodes.filter((n) => n.type === "contributor" || n.type === filterType);
    }
    if (searchQuery.trim()) {
  ...
  useEffect(() => {
    if (!svgRef.current || !gRef.current || filteredData.nodes.length === 0) return;

    const width = containerRef.current?.clientWidth || 800;
    const height = 520;

    // Deep clone nodes and links so D3 simulation doesn't mutate React state directly
    const nodes: (NetworkNode & d3Force.SimulationNodeDatum)[] = filteredData.nodes.map((n) => ({
      ...n,
    }));
    const links: (d3Force.SimulationLinkDatum<NetworkNode & d3Force.SimulationNodeDatum> & { weight: number })[] =
    ...
    const simulation = d3Force
      ...
      .force(
        "collide",
        d3Force.forceCollide<NetworkNode & d3Force.SimulationNodeDatum>((d) => d.val + 14)
      );
    ...
    const linkGroup = g
      ...
      .attr("stroke-dasharray", (d: any) => (d.target.type === "collaborator" ? "4,4" : "none"));
  ...
  const handleResetZoom = useCallback(() => {
    if (!svgRef.current) return;
    const svg = d3Selection.select(svgRef.current);
    svg.call(d3Zoom.zoom<SVGSVGElement, unknown>().transform as any, d3Zoom.zoomIdentity);
  }, []);
  ...
  return (
    ...
    >
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: "6px",
                border: "1px solid",
                borderColor: filterType === tab.key ? "var(--color-hairline-strong)" : "transparent",
                backgroundColor: filterType === tab.key ? "var(--color-canvas)" : "transparent",
                color: filterType === tab.key ? "var(--color-ink)" : "var(--color-ink-mute)",
                cursor: "pointer",
                display: "inline-flex",
                alignItems: "center",
                gap: "6px",
      ...
      >
        ...
        >
          <div style={{ fontWeight: 600, fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.5px", color: "#94a3b8" }}>
            Network Nodes
          </div>
        ...
        {activeTooltipNode && (
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: "11px", color: "#9ca3af" }}>Click to visit</span>
            </div>

            <div style={{ fontWeight: 600, fontSize: "14px", color: "#ffffff", marginTop: "2px" }}>
              {activeTooltipNode.label}
            </div>

            {activeTooltipNode.details && (
              <p style={{ fontSize: "12px", color: "#d1d5db", margin: 0, lineHeight: 1.4 }}>
                {activeTooltipNode.details}
              </p>
            )}
