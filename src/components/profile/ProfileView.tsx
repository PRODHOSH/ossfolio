...
export function ProfileView({
  ...
  const hasFunding = !!(
    sponsorshipData &&
    (sponsorshipData.fundingLinks.length > 0 ||
  ...
  const achievementsWithDates = useMemo(() => {
    return achievements.map((a) => ({
      ...
      unlockedAt:
        unlockedMap[a.id] ||
        (a.unlocked ? new Date().toISOString() : undefined),
    }));
  ...
  const handleRefresh = async () => {
    ...
    setRefreshAnnouncement(
      `Refreshing GitHub profile statistics for @${user.login}...`,
    );
    try {
  ...
  return (
    ...
    >
        ...
        <div style={{ flex: 1, minWidth: '200px' }}>
          ...
          {(() => {
            ...
            return (
              ...
              >
                <ContributorBadge tier={tier}>{tierName}</ContributorBadge>
              </div>
            );
          })()}
                    ...
                    <Tooltip.Root key={badge.program}>
                      <Tooltip.Trigger asChild>
                        ...
                        >
                          ...
                          <span className="bg-white/25 px-1.5 py-0.5 rounded-full text-[11px] font-medium">
                            {badge.years.join(', ')}
                          </span>
                          {isOwner && (
                            <button
                              ...
                              {isOwner && (
                                ...
                                >
                                  <svg
                                    ...
                                    viewBox="0 0 24 24"
                                    fill={
                                      isPinnedRepo ? 'currentColor' : 'none'
                                    }
                                    stroke="currentColor"
                                    strokeWidth="2"
                                  >
      ...
      {/* Sponsorship & Funding */}
      {sponsorshipData && (
        <SponsorshipSection sponsorshipData={sponsorshipData} />
      )}
