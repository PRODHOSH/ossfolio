...
const PLATFORM_COLORS: Record<FundingLink["platform"], { bg: string; color: string; border: string }> = {
  "GitHub Sponsors": {
...
export function SponsorshipSection({ sponsorshipData }: SponsorshipSectionProps) {
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div>
            ...
            >
              Support @{username}&apos;s open source work directly or join as a sponsor
            </p>
          </div>
        </div>
      </div>
      ...
      {fundingLinks.length > 0 && (
        ...
        >
          {fundingLinks.map((link, idx) => {
            const styles = PLATFORM_COLORS[link.platform] || PLATFORM_COLORS.Custom;
            return (
              <a
                key={idx}
                href={link.url}
