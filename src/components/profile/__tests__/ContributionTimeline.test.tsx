...
describe('ContributionTimeline', () => {
  it('renders timeline title and all activity events by default', () => {
    ...
    expect(
      screen.getByText('Fix bug in authentication flow'),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Created \/ Contributed to react-cool-library/),
    ).toBeInTheDocument();
    expect(screen.getByText(/Joined Organization: Vercel/)).toBeInTheDocument();
    expect(
      screen.getByText(/Earned Hacktoberfest Badge \(2025\)/),
    ).toBeInTheDocument();
  });
