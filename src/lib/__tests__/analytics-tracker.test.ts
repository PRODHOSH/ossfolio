...
describe('analytics-tracker helpers', () => {
  describe('parseReferrer', () => {
    ...
    it('should recognize major social platforms & search engines', () => {
      ...
      expect(parseReferrer('https://www.google.com/search?q=ossfolio')).toBe(
        'Search Engine',
      );
      expect(parseReferrer('https://news.ycombinator.com/item?id=123')).toBe(
        'Hacker News',
      );
    ...
    it('should extract domain name for unmapped web sites', () => {
      expect(parseReferrer('https://blog.mywebsite.com/posts/1')).toBe(
        'blog.mywebsite.com',
      );
    });
    ...
    it('should prevent incomplete URL substring spoofing', () => {
      ...
      expect(parseReferrer('https://github.com.attacker.com')).toBe(
        'github.com.attacker.com',
      );
      expect(parseReferrer('https://attacker.com?ref=github.com')).toBe(
        'attacker.com',
      );
      expect(parseReferrer('https://evil-twitter.com')).toBe(
        'evil-twitter.com',
      );
      expect(parseReferrer('https://attacker.com/linkedin.com')).toBe(
        'attacker.com',
      );
      expect(parseReferrer('https://fakebing.com')).toBe('fakebing.com');
      expect(parseReferrer('https://fakeduckduckgo.com')).toBe(
        'fakeduckduckgo.com',
      );
      expect(parseReferrer('https://fakeyahoo.com')).toBe('fakeyahoo.com');
      expect(parseReferrer('https://fakereddit.com')).toBe('fakereddit.com');
      expect(parseReferrer('https://fakeycombinator.com')).toBe(
        'fakeycombinator.com',
      );
      expect(parseReferrer('https://fakehashnode.com')).toBe(
        'fakehashnode.com',
      );
