...
describe("badges-config criteria evaluation", () => {
  ...
  it("evaluates 'first_100_commits' correctly", () => {
    ...
    expect(criterion.evaluate({ stats: { totalCommits: 50, totalPRs: 0, totalIssues: 0, totalReviews: 0 } })).toBe(false);
    expect(criterion.evaluate({ stats: { totalCommits: 100, totalPRs: 0, totalIssues: 0, totalReviews: 0 } })).toBe(true);
  });

  it("evaluates 'century_prs' correctly", () => {
    const criterion = getCriterion("century_prs");
    expect(criterion.evaluate({ stats: { totalCommits: 0, totalPRs: 99, totalIssues: 0, totalReviews: 0 } })).toBe(false);
    expect(criterion.evaluate({ stats: { totalCommits: 0, totalPRs: 100, totalIssues: 0, totalReviews: 0 } })).toBe(true);
  });
  ...
  it("evaluates 'bug_squasher' correctly", () => {
    ...
    expect(criterion.evaluate({ stats: { totalIssues: 5, totalCommits: 0, totalPRs: 0, totalReviews: 0 } })).toBe(false);
    expect(criterion.evaluate({ stats: { totalIssues: 10, totalCommits: 0, totalPRs: 0, totalReviews: 0 } })).toBe(true);
  });
...
describe("awardBadges function", () => {
  it("awards new automated badges while preserving existing user program badges", () => {
    const existingBadges: BadgeItem[] = [
      { program: "GSSoC", years: [2025] },
    ];

    const inputData = {
      stats: { totalCommits: 150, totalPRs: 120, totalIssues: 15, totalReviews: 60 },
      repos: [
