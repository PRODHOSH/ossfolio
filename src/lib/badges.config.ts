 ...
 * 1. awardBadges(existingBadges: BadgeItem[], input: AutomatedBadgeInput): BadgeItem[]
 * 2. awardBadges(userId: string, input: AutomatedBadgeInput | ContributorStats): Promise<BadgeItem[]>
 */
...
export function awardBadges(
  userId: string,
  input: AutomatedBadgeInput | ContributorStats,
): Promise<BadgeItem[]>;
export function awardBadges(
  ...
  if (typeof firstArg === 'string') {
    return (async () => {
      ...
      return evaluateAndAward(existingBadges, input);
    })();
  }
