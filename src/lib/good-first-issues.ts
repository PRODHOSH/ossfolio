...
export const FALLBACK_GOOD_FIRST_ISSUES: GoodFirstIssue[] = [
  ...
  {
    id: "issue-2",
    title: "feat(ui): Add dark mode toggle accessibility aria-label to Button component",
  ...
  {
    id: "issue-3",
    title: "fix(cli): Improve error message formatting when package config is missing",
      ...
      if (res.ok) {
        ...
        issues = rawItems.map((item: any) => {
          const repoUrl = item.repository_url
            ? item.repository_url.replace("https://api.github.com/repos/", "https://github.com/")
            : "https://github.com";
