...
export async function refreshOrganizationStats(
  ...
  try {
    const [orgRes, reposRes, membersRes] = await Promise.allSettled([
      fetchWithTimeout(
        `${GITHUB_API_URL}/orgs/${encodeURIComponent(cleanSlug)}`,
        {
          headers: { Accept: 'application/vnd.github.v3+json' },
        },
      ),
      fetchWithTimeout(
        `${GITHUB_API_URL}/orgs/${encodeURIComponent(cleanSlug)}/repos?per_page=30&sort=updated`,
        {
          headers: { Accept: 'application/vnd.github.v3+json' },
        },
      ),
      fetchWithTimeout(
        `${GITHUB_API_URL}/orgs/${encodeURIComponent(cleanSlug)}/public_members?per_page=30`,
        {
          headers: { Accept: 'application/vnd.github.v3+json' },
        },
      ),
